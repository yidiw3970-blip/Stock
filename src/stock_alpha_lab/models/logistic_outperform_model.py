"""Walk-forward logistic outperform prediction research helpers."""
...
def prepare_outperform_dataset(
    factor_df: pd.DataFrame,
    forward_return_df: pd.DataFrame,
    factor_names: list[str],
    horizon: int,
    top_quantile: float = 0.80,
    target_col_name: str = "outperform_target",
) -> pd.DataFrame:
    """Prepare a wide logistic dataset with same-date outperform labels."""

    resolved_factor_names = _resolve_factor_names(factor_names)
    _validate_top_quantile(top_quantile)
    ...
    target = _build_outperform_targets(
...
def fit_predict_logistic_walk_forward(
    dataset: pd.DataFrame,
    feature_cols: list[str],
    target_col: str = "outperform_target",
    date_col: str = "date",
    ...
    """Fit logistic models through chronological walk-forward splits."""
    ...
    for split in splits:
        ...
        if target_values.nunique(dropna=True) < 2:
            LOGGER.warning(
                "Skipping split %s because logistic training target has one class.",
                split["split_id"],
            )
            continue
        ...
        model.fit(x_train, target_values.astype("int64"))
    ...
    if not prediction_frames:
        raise ValueError("No logistic walk-forward predictions were produced.")

    return (
        pd.concat(prediction_frames, ignore_index=True)
        .loc[:, LOGISTIC_OUTPUT_COLUMNS]
        .sort_values(["date", "split_id", "ticker"])
        .reset_index(drop=True)
    )


def _build_outperform_targets(
    forward_returns: pd.DataFrame,
    horizon: int,
    top_quantile: float,
    target_col_name: str,
) -> pd.DataFrame:
    filtered = forward_returns[forward_returns["horizon"] == int(horizon)].dropna(
        subset=["forward_return"]
    )
    if filtered.empty:
        return pd.DataFrame(columns=["date", "ticker", target_col_name])

    frames = []
    for _, group in filtered.groupby("date", sort=True):
        threshold = group["forward_return"].quantile(top_quantile)
        target = group.loc[:, ["date", "ticker"]].copy()
        target[target_col_name] = (
            group["forward_return"] >= threshold
        ).astype("int64")
    ...
    return pd.DataFrame(
        {
            ...
            "outperform_probability": probabilities,
            "split_id": split["split_id"],
            "train_start": split["train_start"],
            "train_end": split["train_end"],
            "test_start": split["test_start"],
            "test_end": split["test_end"],
            "model_type": "logistic_outperform",
            "C": C,
        },
        index=test_df.index,
    )
...
def _validate_top_quantile(top_quantile: float) -> None:
